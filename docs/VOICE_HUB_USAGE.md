...
2. 按 `voice` 读取音色风格配置。
3. 应用禁用词、替换词、断句规则。
4. 长文本按“目标 80 字、最多 120 字”自动分块，优先在句号、问号、感叹号、分号、逗号等停顿处截断。
5. 多段 WAV 自动拼接。
...
返回里的 `duration` 是真实音频时长，单位毫秒；`generation_duration` 是本次请求生成耗时，单位毫秒。不要把生成耗时当成音频长度。
...
语音中心会自动按“目标 80 字、最多 120 字”智能分块，优先找自然停顿点。为了速度和断句自然，调用方最好每次发送 20 到 80 个中文字符。如果是很长回复，建议调用方先按句号、问号、感叹号、逗号拆成多段。
